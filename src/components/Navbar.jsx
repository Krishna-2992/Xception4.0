...
const Navbar = ({active}) => {
  ...
  const NavbarItem = ({ title, classProps}) => {
    return(
      <li className={`mx-4 hover:underline cursor-pointer ${classProps} ${title === active && 'white-glassmorphism p-3'}`}>
      {
        title==='Home' ? <Link to={`/`}>{title}</Link>:<Link to={`/${title}`}>{title}</Link>
      }
      </li>
    )
  }
  ...
  return(
    <nav className="w-full flex md:justify-center justify-between items-center p-4 white-glassmorphism-no-border" >
      ...
      <div className="flex relative">
        ...
        {
          toggleMenu && (
            <ul className="z-50 fixed top-0 -right-2 p-3 w-[70w] h-screen shadow-2xl md:hidden white-glassmorphism-dark
